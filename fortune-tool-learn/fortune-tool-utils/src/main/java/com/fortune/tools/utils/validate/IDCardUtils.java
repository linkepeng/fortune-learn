...
/**
 * 功能描述：  公民身份号码是特征组合码,由十七位数字本体码和一位数字校验码组成.排列顺序从左至右依次为:
...
public class IDCardUtils {
     ...
     * 身份证为15位
     */
    private static final int ID_CARD_LENGTH_FIFTEEN  = 15;

    /**
     * 身份证为18位
     */
    private static final int ID_CARD_LENGTH_EIGHTEEN = 18;
    ...
    public static boolean verify(String idcard) {
        if (idcard.length() == ID_CARD_LENGTH_FIFTEEN) {
            idcard = update2eighteen(idcard);
        }
        if (idcard.length() != ID_CARD_LENGTH_EIGHTEEN) {
            return false;
        }
        //获取输入身份证上的最后一位，它是校验码
        String checkDigit = idcard.substring(17, 18);
        //比较获取的校验码与本方法生成的校验码是否相等
        String checkDigitVal = getCheckDigit(idcard);
        if (checkDigit.equals(checkDigitVal)) {
            return true;
        }
        return false;
    }
    ...
    private static String update2eighteen(String fifteenCardID) {
        ...
        String checkDigit = getCheckDigit(eighteenCardID);
        eighteenCardID = eighteenCardID + checkDigit;
